...
/**
 * Error New Dialog Controller 
 */
ctrlrs.controller('errorNewDialogCtrl',['$scope','$modalInstance','$translate','data',function($scope,$modalInstance,$translate,data){
	//-- Variables -----//
	...
	$scope.icon = (angular.isDefined(data.fa) && angular.equals(data.fa,true)) ? 'fa fa-warning' : 'glyphicon glyphicon-error-sign-new';
...
/**
 * Success Dialog Controller 
 */
ctrlrs.controller('successDialogCtrl',['$scope','$modalInstance','$translate','data',function($scope,$modalInstance,$translate,data){
	//-- Variables -----//

	$scope.header = (angular.isDefined(data.header)) ? data.header : $translate.instant('DIALOGS_SUCCESS');
	$scope.msg = (angular.isDefined(data.msg)) ? data.msg : $translate.instant('DIALOGS_SUCCESS_MSG');
	$scope.icon = (angular.isDefined(data.fa) && angular.equals(data.fa,true)) ? 'fa fa-check' : 'glyphicon glyphicon-check-white';
...
}]); // end SuccessDialogCtrl
/**
 * Confirm Delete Dialog Controller 
 */
ctrlrs.controller('confirmDeleteDialogCtrl',['$scope','$modalInstance','$translate','data',function($scope,$modalInstance,$translate,data){
	//-- Variables -----//

	$scope.header = (angular.isDefined(data.header)) ? data.header : $translate.instant('DIALOGS_CONFIRMATION_DELETE');
	$scope.msg = (angular.isDefined(data.msg)) ? data.msg : $translate.instant('DIALOGS_CONFIRMATION__DELETE_MSG');
	$scope.icon = (angular.isDefined(data.fa) && angular.equals(data.fa,true)) ? 'fa fa-info' : 'glyphicon glyphicon-warning-sign-new';
...
}]); // end ConfirmDeleteDialogCtrl / dialogs.controllers
/**
	...
	.provider('dialogs',[function(){
		...
		this.$get = ['$modal',function ($modal){
			...
			return {
				 ...
				 * @param	opts	object
				 */
				errorNew : function(header,msg,opts){
					opts = _setOpts(opts);

					return $modal.open({
						templateUrl : '/dialogs/error_new.html',
						controller : 'errorNewDialogCtrl',
						backdrop: opts.bd,
						keyboard: opts.kb,
						windowClass: opts.wc,
						size: opts.ws,
				 ...
				 * @param	opts	object
				 */
				success : function(header,msg,opts){
					opts = _setOpts(opts);
				 ...
				 * @param	opts	object
				 */
				confirmDelete : function(header,msg,opts){
					opts = _setOpts(opts);

					return $modal.open({
						templateUrl : '/dialogs/confirm_delete.html',
...
angular.module('dialogs.main',['dialogs.services','ngSanitize']) // requires angular-sanitize.min.js (ngSanitize) //code.angularjs.org/1.2.1/angular-sanitize.min.js
	...
	.config(['$translateProvider','dialogsProvider',function($translateProvider,dialogsProvider){
		...
		try{
			...
			$translateProvider.translations('en-US',{
	            ...
	            DIALOGS_ERROR_DELETE_MSG: "Unable to Delete Report",
	            DIALOGS_CLOSE: "Close",
	            ...
	            DIALOGS_CONFIRMATION_DELETE: "Are you sure?",
	            DIALOGS_CONFIRMATION_DELETE_MSG: "If you delete the event(s) all data will be removed. Are you sure you want to delete the event(s)?",
	            DIALOGS_CONFIRMATION: "Confirmation",
	            DIALOGS_CONFIRMATION_MSG: "Confirmation required.",
	            DIALOGS_SUCCESS: "Success",
	            DIALOGS_SUCCESS_MSG: "Operation completed successfully",
	            DIALOGS_OK: "OK",
	            DIALOGS_YES: "Yes",
	            DIALOGS_NO: "No",
	...
	.run(['$templateCache','$interpolate',function($templateCache,$interpolate){
    	...
    	$templateCache.put('/dialogs/error_new.html','<div class="modal-header dialog-header-error-new"><a href="" class="closePopup" id="closePopup" ng-click="close()"></a><h4 class="modal-title text-danger-new"><span class="'+startSym+'icon'+endSym+'"></span> <span ng-bind-html="header"></span></h4></div><div class="modal-body text-danger-new" ng-bind-html="msg"> '+startSym+'DIALOGS_ERROR_DELETE_MSG'+endSym+' </div><div class="modal-footer-error-new"><button type="button" class="btn btn-default" ng-click="close()">'+startSym+'"DIALOGS_CLOSE" | translate'+endSym+'</button></div>');
    	...
    	$templateCache.put('/dialogs/success.html','<div class="modal-header dialog-header-success"><a href="" class="closePopup" id="closePopup" ng-click="close()" class="pull-right"></a><h4 class="modal-title text-info-new"><span class="'+startSym+'icon'+endSym+'"></span> '+startSym+'header'+endSym+'</h4></div><div class="modal-body text-info-new" ng-bind-html="msg"></div><div class="modal-footer-success"><button type="button" class="btn btn-primary" ng-click="close()">'+startSym+'"DIALOGS_OK" | translate'+endSym+'</button></div>');
    	$templateCache.put('/dialogs/confirm.html','<div class="modal-header dialog-header-confirm"><button type="button" class="close" ng-click="no()">&times;</button><h4 class="modal-title"><span class="'+startSym+'icon'+endSym+'"></span> '+startSym+'header'+endSym+'</h4></div><div class="modal-body" ng-bind-html="msg"></div><div class="modal-footer"><button type="button" id="buttonYes" class="btn btn-default" ng-click="yes()">'+startSym+'"DIALOGS_YES" | translate'+endSym+'</button><button type="button" class="btn btn-primary" ng-click="no()">'+startSym+'"DIALOGS_NO" | translate'+endSym+'</button></div>');
    	$templateCache.put('/dialogs/confirm_delete.html','<div class="modal-header dialog-header-confirm-delete"><h4 class="modal-title text-warning-new"><span class="'+startSym+'icon'+endSym+'"></span> '+startSym+'header'+endSym+'</h4></div><div class="modal-body" ng-bind-html="msg"></div><div class="modal-footer"><button type="button" style="text-shadow: none;" class="btn btn-default" id="confirmDeleteBtn" ng-click="yes()">'+startSym+'"DIALOGS_DELETE" | translate'+endSym+'</button><button type="button" style="text-shadow: none;" class="btn btn-primary" id="cancelDeleteBtn" ng-click="no()">'+startSym+'"DIALOGS_CANCEL" | translate'+endSym+'</button></div>');
