(function () {
angular
    .module('mainMdl').controller('purgedOrderRetrievalCtrl', purgedOrderRetrievalCtrl);
	
purgedOrderRetrievalCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'purgedOrderRetrievalMainService', 'purgedOrderRetrievalChartOptionService'];
    
	function purgedOrderRetrievalCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("purgedOrderRetrievalCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='PurgedOrderRetrieval';
