(function () {
angular
    .module('mainMdl').controller('DocumentServiceCtrl', DocumentServiceCtrl);
	
DocumentServiceCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'DocumentServiceService', 'DocumentServiceChartOptionService'];
    
	function DocumentServiceCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("DocumentServiceCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='DocumentService';
