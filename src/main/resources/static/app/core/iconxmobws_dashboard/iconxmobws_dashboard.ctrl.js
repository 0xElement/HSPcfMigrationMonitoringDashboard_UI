(function () {
angular
    .module('mainMdl').controller('iconxmobwsCtrl', iconxmobwsCtrl);
	
iconxmobwsCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'iconxmobMainService', 'iconxmobwsChartOptionService'];
    
	function iconxmobwsCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("iconxmobwsCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='ICONXMOBWS';
