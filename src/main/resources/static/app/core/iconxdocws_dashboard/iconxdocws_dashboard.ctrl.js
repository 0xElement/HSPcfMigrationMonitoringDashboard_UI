(function () {
angular
    .module('mainMdl').controller('ICONXDOCWSTC7Ctrl', ICONXDOCWSTC7Ctrl);
	
ICONXDOCWSTC7Ctrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'ICONXDOCWSTC7MainService', 'ICONXDOCWSTC7ChartOptionService'];
    
	function ICONXDOCWSTC7Ctrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("ICONXDOCWSTC7Ctrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='ICONXDOCWSTC7';
