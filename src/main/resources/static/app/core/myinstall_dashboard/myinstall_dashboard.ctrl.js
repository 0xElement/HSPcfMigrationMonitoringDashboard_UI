(function () {
angular
    .module('mainMdl').controller('myinstallCtrl', myinstallCtrl);
	
myinstallCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'myinstallMainService', 'myinstallChartOptionService'];
    
	function myinstallCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("myinstallCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='MyInstall';
