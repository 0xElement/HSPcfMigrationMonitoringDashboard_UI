(function () {
angular
    .module('mainMdl').controller('rapidCtrl', rapidCtrl);
	
	rapidCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'rapidMainService', 'rapidChartOptionService'];
    
	function rapidCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("rapidCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
       ...
       /* ctrl.getDonutOptions = getDonutOptions;
        ...
        ctrl.monthDonutDataTitle = "Success/Failures in Last Month";
        ctrl.errorDonutDataTitle = "Success/Failures in Last Month";
       ...
       /* function getDonutOptions(duration) {
        ...
        $scope.$on('child directive change grid data', function(event, args) {
            ...
            mainService.getApplicationData(args.data.name, ctrl.lastUpdatedDate).then(function(response) {
                ...
                ctrl.monthDonutData = response.data.responseData.applicationDataMap["month"].donutList;
                ctrl.errorDonutData = response.data.responseData.applicationDataMap["error"].donutList;
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='RAPIDPASS';
