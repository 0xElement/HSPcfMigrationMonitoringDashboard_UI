(function () {
angular
    .module('mainMdl').controller('iconxpaymentsCtrl', iconxpaymentsCtrl);
	
iconxpaymentsCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'iconxpaymentsMainService', 'iconxpaymentsChartOptionService'];
    
	function iconxpaymentsCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("iconxpaymentsCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='ICONXPayments';
