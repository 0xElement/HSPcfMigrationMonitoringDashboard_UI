(function () {
angular
    .module('mainMdl').controller('appointmentSchedulerServiceCtrl', appointmentSchedulerServiceCtrl);
	
appointmentSchedulerServiceCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'appointmentSchedulerServiceMainService', 'appointmentSchedulerServiceChartOptionService'];
    
	function appointmentSchedulerServiceCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("appointmentSchedulerServiceCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='AppointmentSchedulerService';
