(function () {
angular
    .module('mainMdl').controller('HDIPCIWebCtrl', HDIPCIWebCtrl);
	
HDIPCIWebCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'HDIPCIWebMainService', 'HDIPCIWebChartOptionService'];
    
	function HDIPCIWebCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("HDIPCIWebCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='HDIPCIWeb';
