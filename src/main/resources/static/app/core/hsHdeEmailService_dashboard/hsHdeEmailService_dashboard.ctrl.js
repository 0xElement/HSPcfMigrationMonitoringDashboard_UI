(function () {
angular
    .module('mainMdl').controller('hsHdeEmailServiceCtrl', hsHdeEmailServiceCtrl);
	
hsHdeEmailServiceCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'hsHdeEmailServiceMainService', 'hsHdeEmailServiceChartOptionService'];
    
	function hsHdeEmailServiceCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("hsHdeEmailServiceCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='HsHdeEmailService';
