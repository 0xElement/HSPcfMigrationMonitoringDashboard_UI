(function () {
angular
    .module('mainMdl').controller('ssshWebservicesCtrl', ssshWebservicesCtrl);
	
ssshWebservicesCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'ssshWebservicesMainService', 'ssshWebservicesChartOptionService'];
    
	function ssshWebservicesCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("ssshWebservicesCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='SSSHWebservices';
