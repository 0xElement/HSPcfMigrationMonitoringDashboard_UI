(function () {
angular
    .module('mainMdl').controller('poupdateCtrl', mainCtrl);
	
	mainCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'poupdateService', 'chartOptionService'];
    
	function mainCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("iconxCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='POUPDATE';
