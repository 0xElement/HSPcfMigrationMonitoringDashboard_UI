(function () {
angular
    .module('mainMdl').controller('renoCtrl', renoCtrl);
	
	renoCtrl.$inject = ['$scope', '$document','$interval', '$timeout', '$window', 'dialogs', 'renoMainService', 'renoChartOptionService'];
    
	function renoCtrl($scope, $document, $interval, $timeout, $window, dialogs, mainService, chartOptionService) {
		console.log("renoCtrl");
        var ctrl = this;
        ctrl.hourDonutData = [];
        ctrl.dayDonutData = [];
        ctrl.weekDonutData = [];
        ...
        function getData(lastUpdatedData) {
        	...
        	mainService.getTreeData(lastUpdatedData).then(function(response) {
                ...
                if(response.data.responseData && response.data.responseData.children.length > 0) {
            		...
            		appData.dashBoardName='RENOWALK';
